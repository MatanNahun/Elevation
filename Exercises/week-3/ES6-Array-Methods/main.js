...
let loweCaseArr = messagesFromDad.map((m) => m.toLowerCase());
console.log(loweCaseArr);
...
if (movies.some((m) => m.studio === "Marvel")) {
  console.log("lets go for a movie");
} else {
...
// ex2:
let startWith5 = users.filter((u) => u.address.zipcode.startsWith("5"));

console.log(startWith5);

// ex3:
let startWith5Id = users
  .filter((u) => u.address.zipcode.startsWith("5"))
  .map((u) => u.id);

console.log(startWith5Id);

// ex4:

const names = users.map((u) => u.name).filter((u) => u.startsWith("C"));
console.log(names);
